<div class="d-flex align-items-center mb-3">
    ...
    <h2 class="page-heading mb-0">Nueva configuración</h2>
</div>
<?php if (!empty($error)): ?><div class="alert alert-danger"><?= htmlspecialchars($error) ?></div><?php endif; ?>
<div class="card shadow-sm" style="max-width:640px;">
    <div class="card-body">
        <form method="POST" action="configuracion.php?accion=crear" novalidate id="frm">
            <div class="mb-3">
                ...
                <div class="form-text">Solo letras, números, guiones y guiones bajos. Ej: <code>max_citas_dia</code>. Debe ser única.</div>
                <div class="invalid-feedback" id="clave-err">La clave es obligatoria y solo puede contener letras, números, _ y -.</div>
            </div>
            <div class="mb-3">
                ...
                <select name="tipo_dato" class="form-select" id="tipo_dato" required>
                    ...
                    <option value="<?= $t ?>" <?= (($_POST['tipo_dato'] ?? '') === $t) ? 'selected' : '' ?>><?= $t ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="mb-3">
                ...
                <div class="form-text" id="valor-hint">Ingresa el valor según el tipo seleccionado.</div>
                <div class="invalid-feedback" id="valor-err">El valor es obligatorio y debe coincidir con el tipo.</div>
            </div>
            <div class="mb-3">
                <label class="form-label fw-semibold">Descripción</label>
                <textarea name="descripcion" class="form-control" rows="2"><?= htmlspecialchars($_POST['descripcion'] ?? '') ?></textarea>
            </div>
...
<script>
const hints = {
    string:  'Cualquier texto. Ej: nombre_clinica → MediCitas',
    int:     'Solo números enteros. Ej: 30',
    boolean: 'Solo "true" o "false".',
    json:    'JSON válido. Ej: {"key":"value"}'
};
document.getElementById('tipo_dato').addEventListener('change', function() {
    document.getElementById('valor-hint').textContent = hints[this.value] || 'Ingresa el valor según el tipo.';
});
