<div class="d-flex align-items-center mb-3">
    ...
    <h2 class="page-heading mb-0">Editar estado de cita</h2>
</div>
<?php if (!empty($error)): ?><div class="alert alert-danger"><?= htmlspecialchars($error) ?></div><?php endif; ?>
<?php if (!$estado): ?>
<div class="alert alert-warning">Estado no encontrado.</div>
<?php else: ?>
<div class="card shadow-sm" style="max-width:600px;">
    <div class="card-body">
        <form method="POST" action="estado_cita.php?accion=actualizar&id=<?= $estado['id_estado'] ?>" novalidate id="frm">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea name="descripcion" class="form-control" rows="3"><?= htmlspecialchars($estado['descripcion'] ?? '') ?></textarea>
            </div>
...
document.getElementById('frm').addEventListener('submit', function(e) {
    ...
    if (c.value.trim() !== '' && !/^#[0-9A-Fa-f]{6}$/.test(c.value.trim())) {
        c.classList.add('is-invalid'); ok = false;
